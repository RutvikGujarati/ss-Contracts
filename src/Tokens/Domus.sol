...
contract Domus is ERC20, Ownable(msg.sender), ReentrancyGuard {
    ...
    modifier onlyGovernance() {
        require(
            ...
            "Domus: You are not authorized to perform this action"
        );
        _;
    }
    ...
    constructor(
        ...
        require(
            ...
            "Domus: Invalid DAV token address"
        );
        require(
            Governance != address(0),
            "Domus: Governance address cannot be zero"
        );
        ...
        maxSupply = 10000000000000 ether;
    ...
    function changeInterval(uint256 newInterval) external onlyGovernance {
        require(newInterval > 0, "Domus: Interval must be greater than zero");
        require(
            newInterval != DECAY_INTERVAL,
            "Domus: New interval must be different from the current"
        );
        require(
            block.timestamp >=
    ...
    function changeDavToken(address newDav) external onlyGovernance {
        require(newDav != address(0), "Domus: Invalid DAV token address");
        require(
            newDav != address(davToken),
            "Domus: New DAV token must be different from the current"
        );
        require(
            block.timestamp >=
                lastDAVUpdate[address(davToken)] + COOLDOWN_PERIOD,
            "Governance update cooldown period not yet passed"
        );
    ...
    function mintReward() external nonReentrant {
        ...
        require(reward > 0, "Domus: No reward to mint");

        uint256 mintableHoldings = cumulativeMintableHoldings[msg.sender];
        require(
            mintableHoldings > 0,
            "Domus: No new holdings to calculate minting"
        );
