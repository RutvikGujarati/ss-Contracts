...
contract Currus is ERC20, Ownable(msg.sender), ReentrancyGuard {
    ...
    modifier onlyGovernance() {
        require(
            ...
            "Currus: You are not authorized to perform this action"
        );
        _;
    }
    ...
    constructor(
        ...
        require(
            ...
            "Currus: Invalid DAV token address"
        );
        require(
            Governance != address(0),
            "Currus: Governance address cannot be zero"
        );
    ...
    function changeInterval(uint256 newInterval) external onlyGovernance {
        require(newInterval > 0, "Currus: Interval must be greater than zero");
        require(
            newInterval != DECAY_INTERVAL,
            "Currus: New interval must be different from the current"
        );
        require(
            block.timestamp >=
    ...
    function changeDavToken(address newDav) external onlyGovernance {
        require(newDav != address(0), "Currus: Invalid DAV token address");
        require(
            newDav != address(davToken),
            "Currus: New DAV token must be different from the current"
        );
        require(
            block.timestamp >=
    ...
    function mintReward() external nonReentrant {
        ...
        require(reward > 0, "Currus: No reward to mint");

        uint256 mintableHoldings = cumulativeMintableHoldings[msg.sender];
        require(
            mintableHoldings > 0,
            "Currus: No new holdings to calculate minting"
        );
