...
contract Layti is ERC20, Ownable(msg.sender), ReentrancyGuard {
    ...
    modifier onlyGovernance() {
        require(
            ...
            "Layti: You are not authorized to perform this action"
        );
        _;
    }
    ...
    constructor(
        ...
        require(
            ...
            "Layti: Invalid DAV token address"
        );
        require(
            Governance != address(0),
            "Layti: Governance address cannot be zero"
        );
    ...
    function changeInterval(uint256 newInterval) external onlyGovernance {
        require(newInterval > 0, "Layti: Interval must be greater than zero");
        require(
            newInterval != DECAY_INTERVAL,
            "Layti: New interval must be different from the current"
        );
        require(
            block.timestamp >=
    ...
    function changeDavToken(address newDav) external onlyGovernance {
        require(newDav != address(0), "Layti: Invalid DAV token address");
        require(
            newDav != address(davToken),
            "Layti: New DAV token must be different from the current"
        );
        require(
    ...
    function mintReward() external nonReentrant {
        ...
        require(reward > 0, "Layti: No reward to mint");

        uint256 mintableHoldings = cumulativeMintableHoldings[msg.sender];
        require(
            mintableHoldings > 0,
            "Layti: No new holdings to calculate minting"
        );
