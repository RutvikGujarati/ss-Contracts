...
contract Sanitas is ERC20, Ownable(msg.sender), ReentrancyGuard {
    ...
    modifier onlyGovernance() {
        require(
            ...
            "Sanitas: You are not authorized to perform this action"
        );
        _;
    }
    ...
    constructor(
        ...
        require(
            ...
            "Sanitas: Invalid DAV token address"
        );
        require(
            Governance != address(0),
            "Sanitas: Governance address cannot be zero"
        );
    ...
    function changeInterval(uint256 newInterval) external onlyGovernance {
        require(newInterval > 0, "Sanitas: Interval must be greater than zero");
        require(
            newInterval != DECAY_INTERVAL,
            "Sanitas: New interval must be different from the current"
        );
        require(
            block.timestamp >=
    ...
    function changeDavToken(address newDav) external onlyGovernance {
        require(newDav != address(0), "Sanitas: Invalid DAV token address");
        require(
            newDav != address(davToken),
            "Sanitas: New DAV token must be different from the current"
        );
        require(
            block.timestamp >=
    ...
    function mintReward() external nonReentrant {
        ...
        require(reward > 0, "Sanitas: No reward to mint");

        uint256 mintableHoldings = cumulativeMintableHoldings[msg.sender];
        require(
            mintableHoldings > 0,
            "Sanitas: No new holdings to calculate minting"
        );
