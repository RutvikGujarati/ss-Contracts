...
contract Valir is ERC20, Ownable(msg.sender), ReentrancyGuard {
    ...
    modifier onlyGovernance() {
        require(
            ...
            "Valir: You are not authorized to perform this action"
        );
        _;
    }
    ...
    constructor(
        ...
        require(
            ...
            "Valir: Invalid DAV token address"
        );
        require(
            Governance != address(0),
            "Valir: Governance address cannot be zero"
        );
    ...
    function changeInterval(uint256 newInterval) external onlyGovernance {
        require(newInterval > 0, "Valir: Interval must be greater than zero");
        require(
            newInterval != DECAY_INTERVAL,
            "Valir: New interval must be different from the current"
        );
        require(
            block.timestamp >=
    ...
    function changeDavToken(address newDav) external onlyGovernance {
        require(newDav != address(0), "Valir: Invalid DAV token address");
        require(
            newDav != address(davToken),
            "Valir: New DAV token must be different from the current"
        );
        require(
            block.timestamp >=
    ...
    function mintReward() external nonReentrant {
        ...
        require(reward > 0, "Valir: No reward to mint");

        uint256 mintableHoldings = cumulativeMintableHoldings[msg.sender];
        require(
            mintableHoldings > 0,
            "Valir: No new holdings to calculate minting"
        );
