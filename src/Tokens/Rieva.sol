...
contract Rieva is ERC20, Ownable(msg.sender), ReentrancyGuard {
    ...
    modifier onlyGovernance() {
        require(
            ...
            "Rieva: You are not authorized to perform this action"
        );
        _;
    }
    ...
    constructor(
        ...
        require(
            ...
            "Rieva: Invalid DAV token address"
        );
        require(
            Governance != address(0),
            "Rieva: Governance address cannot be zero"
        );
    ...
    function changeInterval(uint256 newInterval) external onlyGovernance {
        require(newInterval > 0, "Rieva: Interval must be greater than zero");
        require(
            newInterval != DECAY_INTERVAL,
            "Rieva: New interval must be different from the current"
        );
        require(
            block.timestamp >=
    ...
    function changeDavToken(address newDav) external onlyGovernance {
        require(newDav != address(0), "Rieva: Invalid DAV token address");
        require(
            newDav != address(davToken),
            "Rieva: New DAV token must be different from the current"
        );
        require(
            block.timestamp >=
    ...
    function mintReward() external nonReentrant {
        ...
        require(reward > 0, "Rieva: No reward to mint");

        uint256 mintableHoldings = cumulativeMintableHoldings[msg.sender];
        require(
            mintableHoldings > 0,
            "Rieva: No new holdings to calculate minting"
        );
