...
contract TenDollar is ERC20, Ownable(msg.sender), ReentrancyGuard {
    ...
    constructor(
        ...
        require(
            ...
            "10$: Invalid DAV token address"
        );
        require(
            Governance != address(0),
            "10$: Governance address cannot be zero"
        );
    ...
    function changeInterval(uint256 newInterval) external onlyGovernance {
        require(newInterval > 0, "10$: Interval must be greater than zero");
        require(
            newInterval != DECAY_INTERVAL,
            "10$: New interval must be different from the current"
        );
        require(
            block.timestamp >=
    ...
    function changeDavToken(address newDav) external onlyGovernance {
        require(newDav != address(0), "10$: Invalid DAV token address");
        require(
            newDav != address(davToken),
            "10$: New DAV token must be different from the current"
        );
        require(
            block.timestamp >=
    ...
    function mintReward() external nonReentrant {
        ...
        require(reward > 0, "10$: No reward to mint");

        uint256 mintableHoldings = cumulativeMintableHoldings[msg.sender];
        require(
            mintableHoldings > 0,
            "10$: No new holdings to calculate minting"
        );
