...
import {Currus} from "../Tokens/Currus.sol";
...
contract Ratio_Swapping_Auctions_V1_1 is Ownable(msg.sender), ReentrancyGuard {
    ...
    Currus public currus;
    uint256 public percentage = 1;
    address public currusAddress;
    address private constant BURN_ADDRESS =
        0x0000000000000000000000000000000000000369;

    address public stateToken;
    address public pairAddress; // for currus
    ...
    constructor(
        ...
        currus = Currus(_currus);
        currusAddress = _currus;
        stateToken = state;
    ...
    function getRatioPrice() public view returns (uint256) {
        ...
        uint256 ratio;
        if (token0 == currusAddress && token1 == stateToken) {
            ratio = (uint256(reserve1) * 1e18) / uint256(reserve0);
        } else if (token0 == stateToken && token1 == currusAddress) {
            ratio = (uint256(reserve0) * 1e18) / uint256(reserve1);
        } else {
            revert("Invalid pair, does not match currus/PSTATE");
        }
    ...
    function depositTokens(
        ...
        require(token != address(0), "currus: Invalid token address");
        require(amount > 0, "currus: Cannot deposit zero tokens");
    ...
    function startAuction() public onlyGovernance {
        require(
            currusAddress != address(0) && stateToken != address(0),
            "Invalid token addresses"
        );

        uint256 currentTime = block.timestamp;

        AuctionCycle storage cycle = auctionCycles[currusAddress][stateToken];
        ...
        auctionCycles[stateToken][currusAddress] = AuctionCycle({
    ...
    function swapTokens(address user) public nonReentrant {
        ...
        address inputToken = currusAddress;
    ...
    function getUserHasSwapped(address user) public view returns (bool) {
        ...
        return
            userSwapTotalInfo[user][currusAddress][stateToken][getCycle]
                .hasSwapped;
    }
    ...
    function isAuctionActive() public view returns (bool) {
        AuctionCycle memory cycle = auctionCycles[currusAddress][stateToken];
    ...
    function calculateAuctionEligibleAmount() public view returns (uint256) {
        ...
        uint256 firstCal = (currus.getMax_supply() * percentage) / 100 ether;
    ...
    function getOutPutAmount() public view returns (uint256) {
        /**
         * @dev Retrieves the current price ratio of currus to PSTATE from the liquidity pool.
         * This ratio determines the value of 1 currus in terms of PSTATE.
         */
        uint256 currentRatio = getRatioPrice();
        ...
        if (isReverseActive) {
             ...
             * - This **intentionally** provides **half the currus rewards** to ensure controlled burning.
             * - Instead of using the inverse ratio (STATE/currus), we use currus/STATE because:
             *   - The goal is to gradually reduce STATE supply without over-rewarding burns.
             *   - This **prevents excessive currus issuance**, keeping the auction balanced.
             * - Example:
             *   - Market ratio = **2 currus/STATE** (1 STATE = **0.5 currus**).
             *   - Instead of giving **2 currus per STATE burned**, we **intentionally give 1 currus**.
             ...
             * - This drives STATE liquidity to external DEXes, increasing currus demand.
