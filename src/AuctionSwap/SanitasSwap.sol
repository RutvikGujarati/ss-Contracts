...
import {Sanitas} from "../Tokens/Sanitas.sol";
...
contract Ratio_Swapping_Auctions_V1_1 is Ownable(msg.sender), ReentrancyGuard {
    ...
    Sanitas public sanitas;
    uint256 public percentage = 1;
    address public sanitasAddress;
    address private constant BURN_ADDRESS =
        0x0000000000000000000000000000000000000369;

    address public stateToken;
    address public pairAddress; // for sanitas
    ...
    event TokensSwapped(
        ...
        address indexed sanitasAddress,
    ...
    constructor(
        ...
        sanitas = Sanitas(_sanitas);
        sanitasAddress = _sanitas;
        stateToken = state;
    ...
    function getRatioPrice() public view returns (uint256) {
        ...
        uint256 ratio;
        if (token0 == sanitasAddress && token1 == stateToken) {
            ratio = (uint256(reserve1) * 1e18) / uint256(reserve0);
        } else if (token0 == stateToken && token1 == sanitasAddress) {
            ratio = (uint256(reserve0) * 1e18) / uint256(reserve1);
        } else {
            revert("Invalid pair, does not match sanitas/PSTATE");
        }
    ...
    function depositTokens(
        ...
        require(token != address(0), "sanitas: Invalid token address");
        require(amount > 0, "sanitas: Cannot deposit zero tokens");
    ...
    function startAuction() public onlyGovernance {
        require(
            sanitasAddress != address(0) && stateToken != address(0),
            "Invalid token addresses"
        );

        uint256 currentTime = block.timestamp;

        AuctionCycle storage cycle = auctionCycles[sanitasAddress][stateToken];
        ...
        auctionCycles[stateToken][sanitasAddress] = AuctionCycle({
    ...
    function swapTokens(address user) public nonReentrant {
        ...
        address inputToken = sanitasAddress;
    ...
    function getUserHasSwapped(address user) public view returns (bool) {
        ...
        return
            userSwapTotalInfo[user][sanitasAddress][stateToken][getCycle]
                .hasSwapped;
    }
    ...
    function isAuctionActive() public view returns (bool) {
        AuctionCycle memory cycle = auctionCycles[sanitasAddress][stateToken];
    ...
    function calculateAuctionEligibleAmount() public view returns (uint256) {
        ...
        uint256 firstCal = (sanitas.getMax_supply() * percentage) / 100 ether;
    ...
    function getOutPutAmount() public view returns (uint256) {
        /**
         * @dev Retrieves the current price ratio of sanitas to PSTATE from the liquidity pool.
         * This ratio determines the value of 1 sanitas in terms of PSTATE.
         */
        uint256 currentRatio = getRatioPrice();
        ...
        if (isReverseActive) {
             ...
             * - This **intentionally** provides **half the sanitas rewards** to ensure controlled burning.
             * - Instead of using the inverse ratio (STATE/sanitas), we use sanitas/STATE because:
             *   - The goal is to gradually reduce STATE supply without over-rewarding burns.
             *   - This **prevents excessive sanitas issuance**, keeping the auction balanced.
             * - Example:
             *   - Market ratio = **2 sanitas/STATE** (1 STATE = **0.5 sanitas**).
             *   - Instead of giving **2 sanitas per STATE burned**, we **intentionally give 1 sanitas**.
             ...
             * - This drives STATE liquidity to external DEXes, increasing sanitas demand.
