...
import {Valir} from "../Tokens/Valir.sol";
...
contract Ratio_Swapping_Auctions_V1_1 is Ownable(msg.sender), ReentrancyGuard {
    ...
    address public valirAddress;
    address private constant BURN_ADDRESS =
        0x0000000000000000000000000000000000000369;

    address public stateToken;
    address public pairAddress; // for valir
    ...
    function getRatioPrice() public view returns (uint256) {
        ...
        uint256 ratio;
        if (token0 == valirAddress && token1 == stateToken) {
            ratio = (uint256(reserve1) * 1e18) / uint256(reserve0);
        } else if (token0 == stateToken && token1 == valirAddress) {
            ratio = (uint256(reserve0) * 1e18) / uint256(reserve1);
        } else {
            revert("Invalid pair, does not match valir/PSTATE");
        }
    ...
    function depositTokens(
        ...
        require(token != address(0), "valir: Invalid token address");
        require(amount > 0, "valir: Cannot deposit zero tokens");
    ...
    function startAuction() public onlyGovernance {
        require(
            valirAddress != address(0) && stateToken != address(0),
            "Invalid token addresses"
        );

        uint256 currentTime = block.timestamp;

        AuctionCycle storage cycle = auctionCycles[valirAddress][stateToken];
        ...
        auctionCycles[stateToken][valirAddress] = AuctionCycle({
    ...
    function swapTokens(address user) public nonReentrant {
        ...
        address inputToken = valirAddress;
    ...
    function getUserHasSwapped(address user) public view returns (bool) {
        ...
        return
            userSwapTotalInfo[user][valirAddress][stateToken][getCycle]
                .hasSwapped;
    }
    ...
    function isAuctionActive() public view returns (bool) {
        AuctionCycle memory cycle = auctionCycles[valirAddress][stateToken];
    ...
    function calculateAuctionEligibleAmount() public view returns (uint256) {
        ...
        uint256 firstCal = (valir.getMax_supply() * percentage) / 100 ether;
    ...
    function getOutPutAmount() public view returns (uint256) {
        /**
         * @dev Retrieves the current price ratio of valir to PSTATE from the liquidity pool.
         * This ratio determines the value of 1 valir in terms of PSTATE.
         */
        uint256 currentRatio = getRatioPrice();
        ...
        if (isReverseActive) {
             ...
             * - This **intentionally** provides **half the valir rewards** to ensure controlled burning.
             * - Instead of using the inverse ratio (STATE/valir), we use valir/STATE because:
             *   - The goal is to gradually reduce STATE supply without over-rewarding burns.
             *   - This **prevents excessive valir issuance**, keeping the auction balanced.
             * - Example:
             *   - Market ratio = **2 valir/STATE** (1 STATE = **0.5 valir**).
             *   - Instead of giving **2 valir per STATE burned**, we **intentionally give 1 valir**.
             ...
             * - This drives STATE liquidity to external DEXes, increasing valir demand.
