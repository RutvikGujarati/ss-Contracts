...
import {Rieva} from "../Tokens/Rieva.sol";
...
contract Ratio_Swapping_Auctions_V1_1 is Ownable(msg.sender), ReentrancyGuard {
    ...
    address public rievaAddress;
    address private constant BURN_ADDRESS =
        0x0000000000000000000000000000000000000369;

    address public stateToken;
    address public pairAddress; // for rieva
    ...
    function getRatioPrice() public view returns (uint256) {
        ...
        uint256 ratio;
        if (token0 == rievaAddress && token1 == stateToken) {
            ratio = (uint256(reserve1) * 1e18) / uint256(reserve0);
        } else if (token0 == stateToken && token1 == rievaAddress) {
            ratio = (uint256(reserve0) * 1e18) / uint256(reserve1);
        } else {
            revert("Invalid pair, does not match rieva/PSTATE");
        }
    ...
    function depositTokens(
        ...
        require(token != address(0), "rieva: Invalid token address");
        require(amount > 0, "rieva: Cannot deposit zero tokens");
    ...
    function startAuction() public onlyGovernance {
        require(
            rievaAddress != address(0) && stateToken != address(0),
            "Invalid token addresses"
        );

        uint256 currentTime = block.timestamp;

        AuctionCycle storage cycle = auctionCycles[rievaAddress][stateToken];
        ...
        auctionCycles[stateToken][rievaAddress] = AuctionCycle({
    ...
    function swapTokens(address user) public nonReentrant {
        ...
        address inputToken = rievaAddress;
    ...
    function getUserHasSwapped(address user) public view returns (bool) {
        ...
        return
            userSwapTotalInfo[user][rievaAddress][stateToken][getCycle]
                .hasSwapped;
    }
    ...
    function isAuctionActive() public view returns (bool) {
        AuctionCycle memory cycle = auctionCycles[rievaAddress][stateToken];
    ...
    function calculateAuctionEligibleAmount() public view returns (uint256) {
        ...
        uint256 firstCal = (rieva.getMax_supply() * percentage) / 100 ether;
    ...
    function getOutPutAmount() public view returns (uint256) {
        /**
         * @dev Retrieves the current price ratio of rieva to PSTATE from the liquidity pool.
         * This ratio determines the value of 1 rieva in terms of PSTATE.
         */
        uint256 currentRatio = getRatioPrice();
        ...
        if (isReverseActive) {
             ...
             * - This **intentionally** provides **half the rieva rewards** to ensure controlled burning.
             * - Instead of using the inverse ratio (STATE/rieva), we use rieva/STATE because:
             *   - The goal is to gradually reduce STATE supply without over-rewarding burns.
             *   - This **prevents excessive rieva issuance**, keeping the auction balanced.
             * - Example:
             *   - Market ratio = **2 rieva/STATE** (1 STATE = **0.5 rieva**).
             *   - Instead of giving **2 rieva per STATE burned**, we **intentionally give 1 rieva**.
             ...
             * - This drives STATE liquidity to external DEXes, increasing rieva demand.
