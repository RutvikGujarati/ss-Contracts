...
import {Teeah} from "../Tokens/Teeah.sol";
...
contract Ratio_Swapping_Auctions_V1_1 is Ownable(msg.sender), ReentrancyGuard {
    ...
    address public teeahAddress;
    address private constant BURN_ADDRESS =
        0x0000000000000000000000000000000000000369;

    address public stateToken;
    address public pairAddress; // for teeah
    ...
    function getRatioPrice() public view returns (uint256) {
        ...
        uint256 ratio;
        if (token0 == teeahAddress && token1 == stateToken) {
            ratio = (uint256(reserve1) * 1e18) / uint256(reserve0);
        } else if (token0 == stateToken && token1 == teeahAddress) {
            ratio = (uint256(reserve0) * 1e18) / uint256(reserve1);
        } else {
            revert("Invalid pair, does not match teeah/PSTATE");
        }
    ...
    function depositTokens(
        ...
        require(token != address(0), "teeah: Invalid token address");
        require(amount > 0, "teeah: Cannot deposit zero tokens");
    ...
    function startAuction() public onlyGovernance {
        require(
            teeahAddress != address(0) && stateToken != address(0),
            "Invalid token addresses"
        );

        uint256 currentTime = block.timestamp;

        AuctionCycle storage cycle = auctionCycles[teeahAddress][stateToken];
        ...
        auctionCycles[stateToken][teeahAddress] = AuctionCycle({
    ...
    function swapTokens(address user) public nonReentrant {
        ...
        address inputToken = teeahAddress;
    ...
    function getUserHasSwapped(address user) public view returns (bool) {
        ...
        return
            userSwapTotalInfo[user][teeahAddress][stateToken][getCycle]
                .hasSwapped;
    }
    ...
    function isAuctionActive() public view returns (bool) {
        AuctionCycle memory cycle = auctionCycles[teeahAddress][stateToken];
    ...
    function calculateAuctionEligibleAmount() public view returns (uint256) {
        ...
        uint256 firstCal = (teeah.getMax_supply() * percentage) / 100 ether;
    ...
    function getOutPutAmount() public view returns (uint256) {
        /**
         * @dev Retrieves the current price ratio of teeah to PSTATE from the liquidity pool.
         * This ratio determines the value of 1 teeah in terms of PSTATE.
         */
        uint256 currentRatio = getRatioPrice();
        ...
        if (isReverseActive) {
             ...
             * - This **intentionally** provides **half the teeah rewards** to ensure controlled burning.
             * - Instead of using the inverse ratio (STATE/teeah), we use teeah/STATE because:
             *   - The goal is to gradually reduce STATE supply without over-rewarding burns.
             *   - This **prevents excessive teeah issuance**, keeping the auction balanced.
             * - Example:
             *   - Market ratio = **2 teeah/STATE** (1 STATE = **0.5 teeah**).
             *   - Instead of giving **2 teeah per STATE burned**, we **intentionally give 1 teeah**.
             ...
             * - This drives STATE liquidity to external DEXes, increasing teeah demand.
