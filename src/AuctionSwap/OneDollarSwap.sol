...
import {OneDollar} from "../Tokens/OneDollar.sol";
...
contract Ratio_Swapping_Auctions_V1_1 is Ownable(msg.sender), ReentrancyGuard {
    ...
    event TokensSwapped(
        ...
        address indexed oneDollarAddress,
    ...
    constructor(
        ...
        oneDollar = OneDollar(_oneDollar);
        oneDollarAddress = _oneDollar;
        stateToken = state;
    ...
    function getRatioPrice() public view returns (uint256) {
        ...
        uint256 ratio;
        if (token0 == oneDollarAddress && token1 == stateToken) {
            ratio = (uint256(reserve1) * 1e18) / uint256(reserve0);
        } else if (token0 == stateToken && token1 == oneDollarAddress) {
            ratio = (uint256(reserve0) * 1e18) / uint256(reserve1);
        } else {
            revert("Invalid pair, does not match oneDollar/PSTATE");
        }
    ...
    function depositTokens(
        ...
        require(token != address(0), "oneDollar: Invalid token address");
        require(amount > 0, "oneDollar: Cannot deposit zero tokens");
    ...
    function startAuction() public onlyGovernance {
        require(
            oneDollarAddress != address(0) && stateToken != address(0),
            "Invalid token addresses"
        );

        uint256 currentTime = block.timestamp;

        AuctionCycle storage cycle = auctionCycles[oneDollarAddress][stateToken];
        ...
        auctionCycles[stateToken][oneDollarAddress] = AuctionCycle({
    ...
    function swapTokens(address user) public nonReentrant {
        ...
        address inputToken = oneDollarAddress;
    ...
    function getUserHasSwapped(address user) public view returns (bool) {
        ...
        return
            userSwapTotalInfo[user][oneDollarAddress][stateToken][getCycle]
                .hasSwapped;
    }
    ...
    function isAuctionActive() public view returns (bool) {
        AuctionCycle memory cycle = auctionCycles[oneDollarAddress][stateToken];
    ...
    function calculateAuctionEligibleAmount() public view returns (uint256) {
        ...
        uint256 firstCal = (oneDollar.getMax_supply() * percentage) / 100 ether;
    ...
    function getOutPutAmount() public view returns (uint256) {
        /**
         * @dev Retrieves the current price ratio of oneDollar to PSTATE from the liquidity pool.
         * This ratio determines the value of 1 oneDollar in terms of PSTATE.
         */
        uint256 currentRatio = getRatioPrice();
        ...
        if (isReverseActive) {
             ...
             * - This **intentionally** provides **half the oneDollar rewards** to ensure controlled burning.
             * - Instead of using the inverse ratio (STATE/oneDollar), we use oneDollar/STATE because:
             *   - The goal is to gradually reduce STATE supply without over-rewarding burns.
             *   - This **prevents excessive oneDollar issuance**, keeping the auction balanced.
             * - Example:
             *   - Market ratio = **2 oneDollar/STATE** (1 STATE = **0.5 oneDollar**).
             *   - Instead of giving **2 oneDollar per STATE burned**, we **intentionally give 1 oneDollar**.
             ...
             * - This drives STATE liquidity to external DEXes, increasing oneDollar demand.
