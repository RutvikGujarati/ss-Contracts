...
import {TenDollar} from "../Tokens/TenDollar.sol";
...
contract Ratio_Swapping_Auctions_V1_1 is Ownable(msg.sender), ReentrancyGuard {
    ...
    event TokensSwapped(
        ...
        address indexed tenDollarAddress,
    ...
    constructor(
        ...
        tenDollar = TenDollar(_tenDollar);
        tenDollarAddress = _tenDollar;
        stateToken = state;
    ...
    function getRatioPrice() public view returns (uint256) {
        ...
        uint256 ratio;
        if (token0 == tenDollarAddress && token1 == stateToken) {
            ratio = (uint256(reserve1) * 1e18) / uint256(reserve0);
        } else if (token0 == stateToken && token1 == tenDollarAddress) {
            ratio = (uint256(reserve0) * 1e18) / uint256(reserve1);
        } else {
            revert("Invalid pair, does not match tenDollar/PSTATE");
        }
    ...
    function depositTokens(
        ...
        require(token != address(0), "tenDollar: Invalid token address");
        require(amount > 0, "tenDollar: Cannot deposit zero tokens");
    ...
    function startAuction() public onlyGovernance {
        require(
            tenDollarAddress != address(0) && stateToken != address(0),
            "Invalid token addresses"
        );

        uint256 currentTime = block.timestamp;

        AuctionCycle storage cycle = auctionCycles[tenDollarAddress][stateToken];
        ...
        auctionCycles[stateToken][tenDollarAddress] = AuctionCycle({
    ...
    function swapTokens(address user) public nonReentrant {
        ...
        address inputToken = tenDollarAddress;
    ...
    function getUserHasSwapped(address user) public view returns (bool) {
        ...
        return
            userSwapTotalInfo[user][tenDollarAddress][stateToken][getCycle]
                .hasSwapped;
    }
    ...
    function isAuctionActive() public view returns (bool) {
        AuctionCycle memory cycle = auctionCycles[tenDollarAddress][stateToken];
    ...
    function calculateAuctionEligibleAmount() public view returns (uint256) {
        ...
        uint256 firstCal = (tenDollar.getMax_supply() * percentage) / 100 ether;
    ...
    function getOutPutAmount() public view returns (uint256) {
        /**
         * @dev Retrieves the current price ratio of tenDollar to PSTATE from the liquidity pool.
         * This ratio determines the value of 1 tenDollar in terms of PSTATE.
         */
        uint256 currentRatio = getRatioPrice();
        ...
        if (isReverseActive) {
             ...
             * - This **intentionally** provides **half the tenDollar rewards** to ensure controlled burning.
             * - Instead of using the inverse ratio (STATE/tenDollar), we use tenDollar/STATE because:
             *   - The goal is to gradually reduce STATE supply without over-rewarding burns.
             *   - This **prevents excessive tenDollar issuance**, keeping the auction balanced.
             * - Example:
             *   - Market ratio = **2 tenDollar/STATE** (1 STATE = **0.5 tenDollar**).
             *   - Instead of giving **2 tenDollar per STATE burned**, we **intentionally give 1 tenDollar**.
             ...
             * - This drives STATE liquidity to external DEXes, increasing tenDollar demand.
